...
# Idempotente: verifica pelo metadata source=barber_management antes de criar.
...
BarberManagement::Stripe::Client.with_api_key do
  ...
  existing = Stripe::Product.list(active: true, limit: 100).data.find do |p|
    p.metadata['source'] == 'barber_management'
  end
  ...
  product = Stripe::Product.create(
    ...
    metadata:    { source: 'barber_management', project: 'BarberManagement' }
  )
  ...
  ORBI_PLANS.each do |plan|
    price = Stripe::Price.create(
      ...
      metadata:    { source: 'barber_management', interval_label: plan[:label] }
    )
