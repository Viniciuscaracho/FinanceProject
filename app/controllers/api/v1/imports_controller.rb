    ...
    class ImportsController < ApplicationController
      ...
      def index
        ...
        if params[:state].present?
          state_mapping = { 'waiting' => 3, 'in_progress' => 0, 'done' => 1, 'failed' => 2 }
          state_cd = state_mapping[params[:state]]
          query = query.where(state_cd: state_cd) if state_cd
        end

        if params[:source].present?
          source_mapping = { 'zero_paper' => 0, 'xlsx_contacts' => 1, 'xlsx_default' => 2 }
      ...
      def import_json(import)
        file_url = if import.file.attached?
          begin
            Rails.application.routes.url_helpers.rails_blob_url(import.file, only_path: false)
          rescue StandardError
            Rails.application.routes.url_helpers.rails_blob_path(import.file, only_path: true)
          end
        end
