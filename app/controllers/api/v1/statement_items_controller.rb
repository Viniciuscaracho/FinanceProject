    ...
    class StatementItemsController < ApplicationController
      ...
      def index
        @statement_items = @statement.statement_items
          .includes(:contact, :category, :related_transaction, :bank_account_source, :bank_account_target)
        ...
        render json: {
          statement_items: @statement_items.as_json(
            include: [
              :contact,
              :category,
              :related_transaction,
      ...
      def show
        render json: {
          statement_item: @statement_item.as_json(
            include: [
              :contact,
              :category,
              :related_transaction,
      ...
      def update
        ...
        if result.success?
          render json: {
            statement_item: @statement_item.reload.as_json(
              include: [
                :contact,
                :category,
                :related_transaction,
      ...
      def bulk_confirm
        ...
        items = @statement.statement_items.where(id: item_ids)
        
        items.find_each do |item|
          StatementItems::Confirm.call(statement_item: item)
        end

        render json: {
          message: "#{items.count} item(ns) confirmado(s) com sucesso",
          count: items.count
        }
      end
      ...
      def bulk_ignore
        ...
        items.find_each do |item|
          StatementItems::Ignore.call(statement_item: item)
        end

        render json: {
          message: "#{items.count} item(ns) ignorado(s) com sucesso",
          count: items.count
        }
      end
