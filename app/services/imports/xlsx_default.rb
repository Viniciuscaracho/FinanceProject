...
module Imports
  class XlsxDefault < ApplicationService
    ...
    PAYMENT_METHODS = {
      ...
      'debito automatico' => :direct_debit,
      'promissoria' => :promissory
    }.freeze
    ...
    def call
      ...
      begin
        ...
        import = extract_excel_items(import:)
    ...
    def import_transactions_to_database(import:)
      ...
      import.zero_paper_items.find_each do |zero_paper_item|
        ...
        case formated_value(zero_paper_item.transaction_type)
        when formated_value(TRANSACTION_TYPES[:REVENUE])           # Se for um recebimento
          transactions << build_transaction(item: zero_paper_item, transaction_type: :revenue)

        when formated_value(TRANSACTION_TYPES[:FIXED_EXPENSE])     # Se for uma despesa fixa
          transactions << build_transaction(item: zero_paper_item, transaction_type: :fixed_expense)

        when formated_value(TRANSACTION_TYPES[:VARIABLE_EXPENSE])  # Se for uma despesa variável
          transactions << build_transaction(item: zero_paper_item, transaction_type: :variable_expense)

        when formated_value(TRANSACTION_TYPES[:PAYROLL])           # Se for uma transação de folha de pagamento / pessoas
          transactions << build_transaction(item: zero_paper_item, transaction_type: :payroll)

        when formated_value(TRANSACTION_TYPES[:TAX])               # Se for um imposto
          transactions << build_transaction(item: zero_paper_item, transaction_type: :tax)

        when formated_value(TRANSACTION_TYPES[:TRANSFER_OUT])      # Se for uma transferência de saída
          transactions << build_transfer_transaction(import:, item: zero_paper_item)

        when formated_value(TRANSACTION_TYPES[:TRANSFER_IN])       # Se for uma transferência de saída
    ...
    def extract_excel_items(import:)
      # Import transfers
      import.file.open do |file|
        ...
        worksheet.rows.each_with_index do |row, index|
          if index.zero?
            ...
            import.message = 'Arquivo incompatível! Verifique se o aquivo está no formato original do template.'
            raise StandardError, import.message
          end

          #if file line is blank
    ...
    def is_column_names_valid?(row:)
      strip_value(row[COLUMN_NAMES[:TRANSACTION_TYPE]]) == 'Tipo de Lançamento' &&
        strip_value(row[COLUMN_NAMES[:DUE_DATE]]) == 'Data de pagamento' &&
        strip_value(row[COLUMN_NAMES[:COMPETENCY_DATE]]) == 'Data de competência' &&
        strip_value(row[COLUMN_NAMES[:NAME]]) == 'Descrição' &&
        ...
        strip_value(row[COLUMN_NAMES[:DESCRIPTION]]) == 'Observações' &&
        strip_value(row[COLUMN_NAMES[:BANK_ACCOUNT]]) == 'Contas bancárias' &&
        strip_value(row[COLUMN_NAMES[:DOCUMENT_NUMBER]]) == 'Número de Documento / Nosso número' &&
        strip_value(row[COLUMN_NAMES[:PAYMENT_METHOD]]) == 'Modo de pagamento' &&
        strip_value(row[COLUMN_NAMES[:COST_CENTER]]) == 'Centro de Custo' &&
        strip_value(row[COLUMN_NAMES[:TAGS]]) == 'Tags (Por Marcação)'
    end
    ...
    def extract_paid(row:)
      formated_value(row[COLUMN_NAMES[:PAID]]) == 'sim'
    end
    ...
    def extract_amount(row:)
      ...
      # Remove símbolos e formatação, mantendo apenas números, pontos e vírgulas
      cleaned_amount = amount_str.gsub(/[^\d.,]/, '')

      # Substitui a vírgula pelo ponto para lidar com diferentes formatos
      cleaned_amount = cleaned_amount.gsub(',', '.') if cleaned_amount.include?(',')

      # Verifica se a string pode ser convertida para um número
      if cleaned_amount =~ /\A\d+(\.|,)?\d{0,2}\z/
        # Retorna o valor formatado com duas casas decimais
        formatted_amount = format('%.2f', cleaned_amount.to_f)

        return formatted_amount
      end

      # Retorna 0 se não for um valor válido
      0
    end
    ...
    def extract_payment_method(row:)
      payment_method = formated_value(row[COLUMN_NAMES[:PAYMENT_METHOD]])
    ...
    def formated_value(value)
      val = value.to_s.strip
      downcased_value = val.downcase
      normalized_value = I18n.transliterate(downcased_value) # Normaliza os caracteres acentuados
