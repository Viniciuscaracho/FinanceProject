...
function FoodSearch({ onSelect, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      ...
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 520, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', overflow: 'hidden', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        {/* Tabs */}
          ...
          <div style={{ flexShrink: 0 }}>
            {!showCustomForm ? (
              <button type="button" onClick={() => setShowCustomForm(true)}
                style={{ width: '100%', padding: '8px 14px', border: 'none', borderTop: `1px solid ${T.border}`, background: '#F9FAFB', cursor: 'pointer', fontSize: 12, color: T.brand, fontFamily: 'inherit', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, justifyContent: 'center' }}>
                <Plus size={13} /> Criar novo alimento
              </button>
            ) : (
              <div style={{ borderTop: `1px solid ${T.border}`, padding: '12px 14px', background: '#F9FAFB' }}>
            ...
            <button key={food.id || food.external_id || idx} type="button"
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>
                  ...
                  <span style={{ marginLeft: 5, padding: '0 5px', borderRadius: 4, background: '#F3F4F6', fontSize: 10 }}>{sourceLabel(food.source)}</span>
                </div>
              </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: '1px solid #F3F4F6' }}>
      ...
      <button type="button" onClick={() => onRemove(item.id)}
        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D1D5DB', padding: 6, flexShrink: 0, minWidth: 32, minHeight: 32, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Trash2 size={13} />
      </button>
    </div>
  )
      ...
      <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '8px 12px', background: '#FAFAFA', cursor: 'pointer' }}
          ...
          <button type="button" onClick={e => { e.stopPropagation(); if (window.confirm(`Remover "${meal.name}"?`)) onRemove(meal.id) }}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D1D5DB', padding: 6, marginRight: 2, minWidth: 32, minHeight: 32, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Trash2 size={13} />
          </button>
    ...
    <div style={{ border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', background: T.chip }}>
        ...
        <button type="button" onClick={() => { if (window.confirm(`Remover ${day.label}?`)) onRemoveDay(day.id) }}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D1D5DB', padding: 6, marginRight: 4, minWidth: 32, minHeight: 32, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Trash2 size={14} />
        </button>
...
function TargetsModal({ plan, contactId, planId, onSave, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 460, padding: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
