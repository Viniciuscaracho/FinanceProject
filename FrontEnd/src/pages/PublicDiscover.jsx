...
import { Search, MapPin, Loader2, Navigation, Minus, Plus, Map, LayoutGrid } from 'lucide-react'
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: highlighted ? '0 8px 28px rgba(76,96,170,0.14)' : '0 1px 4px rgba(0,0,0,0.04)',
      ...
      <div style={{ position: 'relative', height: 180, background: T.chip, flexShrink: 0, overflow: 'hidden' }}>
        {professional.logo_url ? (
          ...
          <div style={{
            ...
            background: 'linear-gradient(135deg, #dce5f5 0%, #c9d7f0 100%)',
          }}>
            <span style={{ fontSize: 56, fontWeight: 800, color: T.brand, opacity: 0.45, ...DISPLAY }}>
              {getInitials(professional.name)}
            </span>
          </div>
        )}
        {/* badge de especialidade sobre a foto */}
        <span style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 10 }}>
            {tags.map((t, i) => (
              <span key={i} style={{
                ...
                borderRadius: 5, background: T.light, color: T.muted,
              }}>
                {t}
              </span>
            ))}
    ...
    >
      ...
      <div style={{ width: 46, height: 46, borderRadius: 10, flexShrink: 0, overflow: 'hidden', background: T.chip }}>
        {professional.logo_url ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #dce5f5, #c9d7f0)' }}>
...
function DiscoverMap({ results, onSearch, highlightedId, onHoverPin }) {
  ...
  useEffect(() => {
    ...
    const crossIcon = L.divIcon({
      html: `<div style="width:30px;height:30px;border-radius:50%;border:3px solid #4C60AA;background:rgba(76,96,170,0.15);display:flex;align-items:center;justify-content:center;">
               <div style="width:9px;height:9px;border-radius:50%;background:#4C60AA;"></div>
             </div>`,
    ...
    const circle = L.circle(SAO_PAULO, {
      radius: 10000, color: '#4C60AA', fillColor: '#4C60AA',
  ...
  useEffect(() => {
    ...
    results.forEach(pro => {
      ...
      const icon = L.divIcon({
        html: `<div style="background:${hl ? '#111' : '#4C60AA'};color:#fff;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;font-family:'Space Grotesk',system-ui,sans-serif;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,0.25);border:2px solid #fff;">${pro.name.split(' ')[0]}</div>`,
      ...
      const logoHtml = pro.logo_url
        ...
        : `<div style="width:38px;height:38px;border-radius:7px;background:#EEF2FA;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:#4C60AA;flex-shrink:0;">${getInitials(pro.name)}</div>`
      const popup = `<div style="font-family:'Space Grotesk',system-ui,sans-serif;min-width:190px;padding:2px 0">
        <div style="display:flex;gap:10px;align-items:center;margin-bottom:8px">${logoHtml}
          ...
          <span style="font-size:10px;font-weight:600;color:#4C60AA;background:#EEF2FA;border-radius:4px;padding:1px 6px">Nutricionista</span></div></div>
        ${pro.location?.city ? `<p style="margin:0 0 8px;font-size:11px;color:#6B6B6B">📍 ${[pro.location.district, pro.location.city].filter(Boolean).join(', ')}</p>` : ''}
        <a href="/descobrir/${pro.id}" style="display:block;text-align:center;background:#4C60AA;color:#fff;border-radius:6px;padding:6px 12px;font-size:12px;font-weight:600;text-decoration:none">Ver perfil →</a>
      </div>`
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: `1px solid ${T.border}`, background: T.bg }}>
        <div style={{ padding: '12px 14px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={handleGeo} disabled={geoLoading}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: `1px solid ${T.border}`, background: T.white, color: T.brand, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', flex: 1, justifyContent: 'center' }}>
        ...
        <div style={{ padding: '10px 14px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => setRadius(r => Math.max(1, r - 5))} style={{ width: 22, height: 22, borderRadius: 5, border: `1px solid ${T.border}`, background: T.white, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Minus size={10} /></button>
          <span style={{ fontSize: 12, fontWeight: 700, color: T.brand, minWidth: 40, textAlign: 'center' }}>{radius} km</span>
          <button onClick={() => setRadius(r => Math.min(100, r + 5))} style={{ width: 22, height: 22, borderRadius: 5, border: `1px solid ${T.border}`, background: T.white, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={10} /></button>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {searching ? (
            ...
            <div style={{ textAlign: 'center', paddingTop: 40 }}>
              <p style={{ fontSize: 13, color: T.muted }}>Clique no mapa para buscar nutricionistas na região</p>
            </div>
          ) : (
            results.filter(r => r.location?.latitude).map(pro => (
              <CompactCard key={pro.id} professional={pro} highlighted={highlightedId === pro.id} onHover={onHoverPin} onClick={() => window.location.href = `/descobrir/${pro.id}`} />
            ))
          )}
        </div>
      </div>
...
export function PublicDiscover() {
  ...
  const HEADER_H = 112 // header sticky height (aprox)
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg, ...DISPLAY, overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, background: T.white, borderBottom: `1px solid ${T.border}`, zIndex: 40 }}>
        {/* Row 1: nav + toggle */}
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 13, color: T.muted, fontWeight: 500 }}>Nutricionistas</span>
          {total > 0 && (
            <span style={{ fontSize: 11, color: T.muted, background: T.light, borderRadius: 20, padding: '2px 8px', fontWeight: 600 }}>
              {total} cadastradas
            </span>
          )}
        ...
        {/* Row 2: busca + chips */}
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 12px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <form onSubmit={handleSearch} style={{ display: 'flex', gap: 7, flex: '1 1 300px' }}>
            <div style={{ position: 'relative', flex: '1 1 160px' }}>
              <Search size={13} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: T.muted, pointerEvents: 'none' }} />
              <input type="text" placeholder="Nome" value={searchQ} onChange={e => setSearchQ(e.target.value)}
                style={{ width: '100%', boxSizing: 'border-box', paddingLeft: 29, paddingRight: 10, paddingTop: 7, paddingBottom: 7, border: `1px solid ${T.border}`, borderRadius: 7, fontSize: 13, color: T.text, background: T.white, outline: 'none', fontFamily: 'inherit' }}
            ...
            <div style={{ position: 'relative', flex: '1 1 140px' }}>
              ...
              <input type="text" placeholder="Cidade ou bairro" value={searchCity} onChange={e => setSearchCity(e.target.value)}
                style={{ width: '100%', boxSizing: 'border-box', paddingLeft: 29, paddingRight: 10, paddingTop: 7, paddingBottom: 7, border: `1px solid ${T.border}`, borderRadius: 7, fontSize: 13, color: T.text, background: T.white, outline: 'none', fontFamily: 'inherit' }}
                onFocus={e => { e.target.style.borderColor = T.brand; e.target.style.boxShadow = `0 0 0 3px ${T.chip}` }}
                onBlur={e => { e.target.style.borderColor = T.border; e.target.style.boxShadow = 'none' }} />
            </div>
            <button type="submit" style={{ padding: '7px 16px', background: T.brand, color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
              Buscar
            </button>
          </form>

          {/* Specialty chips */}
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', flexShrink: 0, paddingBottom: 1 }}>
      ...
      {viewMode === 'map' ? (
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <DiscoverMap results={results} onSearch={handleMapSearch} highlightedId={hoveredId} onHoverPin={setHoveredId} />
        </div>
      ) : (
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', paddingTop: 80 }}>
                ...
                <button onClick={() => { setSearchQ(''); setSearchCity(''); setActiveChip(''); fetchPage('', '', '', 0, false) }}
                  style={{ padding: '9px 20px', background: T.brand, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                  Ver todas
                </button>
              </div>
            ) : (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
                  {results.map(pro => (
                    <RichCard key={pro.id} professional={pro} highlighted={hoveredId === pro.id}
                      onClick={() => navigate(`/descobrir/${pro.id}`)} onHover={setHoveredId} />
                  ))}
                </div>
                ...
                {results.length < total && (
                  <div style={{ textAlign: 'center', marginTop: 40 }}>
                    <button onClick={handleLoadMore} disabled={loadingMore}
                      style={{ padding: '10px 28px', border: `1px solid ${T.border}`, borderRadius: 8, background: T.white, color: T.text, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', opacity: loadingMore ? 0.5 : 1, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                      {loadingMore
