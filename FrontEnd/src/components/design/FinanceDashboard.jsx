...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 10, padding: "10px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: T.muted, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</p>
      {payload.map(p => (
        <p key={p.dataKey} style={{ fontSize: 13, fontWeight: 600, color: p.dataKey === "receitas" ? BRAND : T.muted, margin: "2px 0" }}>
...
export default function FinanceDashboard() {
  ...
  return (
    <div style={{ background: T.white, borderRadius: 16, padding: "24px 28px" }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ fontSize: 17, fontWeight: 700, color: T.text, margin: 0, letterSpacing: "-0.02em" }}>
            Controle financeiro
          </h2>
        ...
        {/* Segmented control */}
        <div style={{ display: "flex", gap: 2, background: T.light, borderRadius: 10, padding: 3 }}>
          {["Hoje", "Semana", "Mês"].map(p => (
            <button
              ...
              style={{
                ...
                background: period === p ? T.white : "transparent",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
        {kpis.map((kpi, i) => (
          ...
          >
            <p style={{ fontSize: 11, color: T.muted, fontWeight: 600, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              {kpi.label}
            </p>
            <p style={{ fontSize: 20, fontWeight: 700, color: T.text, margin: "0 0 4px", letterSpacing: "-0.02em" }}>
              {kpi.value}
            </p>
      ...
      {/* Chart */}
      <div style={{ background: T.light, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 20px 12px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: T.text, margin: 0 }}>Receitas × Despesas</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} barGap={4} barCategoryGap="35%">
              ...
              <YAxis
                ...
                width={40}
              />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: T.light, radius: 6 }} />
