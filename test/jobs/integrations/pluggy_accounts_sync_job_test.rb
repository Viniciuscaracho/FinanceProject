...
module Integrations
  class PluggyAccountsSyncJobTest < ActiveSupport::TestCase
    setup do
    ...
    test 'should successfully execute pluggy accounts job' do
      Integrations::PluggyAccountsSync.expects(:call).once.with(account_id: @account.id,
                                                                sync_type: RelationshipStore::SYNC_TYPES[:initial_sync]).returns(
                                                                  OpenStruct.new(
                                                                    success?: true
                                                                  )
                                                                )
      Integrations::PluggyAccountsSyncJob.perform_now(@account.id, RelationshipStore::SYNC_TYPES[:initial_sync])
    end
